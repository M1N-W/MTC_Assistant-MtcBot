...
class ExamSessionManager:
    ...
    def _finish_exam(self, user_id: str) -> str:
        ...
        session = self.get_session(user_id)
        if not session:
            return "ฮึบๆ สัญญาณขาดหาย โหลดผลสอบไม่ได้ซะงั้น 📡 รบกวนกดสอบใหม่อีกรอบนะครับ!"
...
def handle_exam_stats(user_id: str, db=None) -> str:
    ...
    try:
        ...
        return "ฮึบๆ สัญญาณขาดหาย ดึงข้อสอบไม่ได้ซะงั้น 📡 รบกวนกดสอบใหม่อีกรอบนะครับ!"
...
def get_exam_help() -> str:
    ...
    return (
        "🔥 พร้อมจะวัดความรู้หรือยัง!? มาลุยข้อสอบจำลองกันเลย!\n\n"
        "📌 วิธีใช้\n"
        "  สอบ [วิชา] [ระดับ] [จำนวนข้อ]\n"
